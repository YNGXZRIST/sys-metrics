...
func TestParseArgs_hashKey(t *testing.T) {
	opt, err := parseArgs([]string{"-k", "hmac-secret"})
	if err != nil {
		t.Fatal(err)
	}
	if opt.HashKey == nil || *opt.HashKey != "hmac-secret" {
		t.Fatalf("HashKey = %v", opt.HashKey)
	}
}

func TestParseArgs_cryptoKey(t *testing.T) {
	opt, err := parseArgs([]string{"-crypto-key", "/path/to/key.pem"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewOption_development(t *testing.T) {
	for _, k := range []string{
		...
		"DATABASE_DSN", "AUDIT_FILE", "AUDIT_URL", "RESTORE", "CRYPTO_KEY",
	} {
		t.Setenv(k, "")
	}
